...
export const articleModel = {
    getAll: () => {
        const rows = db.prepare('SELECT * FROM articles ORDER BY date DESC, createdAt DESC').all();
        return rows.map(row => ({
            id: row.id,
            title: row.title,
            summary: row.summary,
    ...
    create: (article) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(article.id, article.title, article.summary, article.imageUrl, article.tag, article.date, article.views || 0, article.content || null);
    ...
    update: (id, article) => {
      ...
      WHERE id = ?
    `);
        stmt.run(updated.title, updated.summary, updated.imageUrl, updated.tag, updated.date, updated.views || 0, updated.content || null, id);
